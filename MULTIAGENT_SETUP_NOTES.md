...
- 「起動しただけで勝手に開発が走らない」ように、起動時は **何も送信しない**
...
### ユーザー→PRESIDENT→boss1→worker の流れ
- ユーザーがPRESIDENTに指示を出すまで、勝手に作業指示は送信しない
- 起動直後は **誰にも自動送信しない**（起動のみ）
- 役割プロンプトは必要なときだけ `bootstrap.sh` を手動実行する
...
- `./.ai-team/multiagent/bootstrap.sh`  
  役割プロンプト投入（**boss/workerのみ**。必要なときだけ手動実行）
- `./.ai-team/multiagent/start.sh`  
  `setup.sh` → `launch-agents.sh` を実行し、最後に tmux attach（**自動送信なし**）
...
- `--bootstrap`（必要なときだけ役割プロンプト投入）
- `--bootstrap-delay <seconds>`（投入を遅らせる。デフォルト `2`）
...
- `bootstrap.sh` は **boss/workerへの役割付与のみ**（必要なときだけ手動実行）
- PRESIDENT には自動送信しない（ユーザーが入力）
